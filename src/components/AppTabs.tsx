...
const AppTabs: React.FC = () => {
      ...
      try {
        ...
        const imported = json.map((r) => {
          ...
          allCols.forEach((c) => {
            ...
            row[c] =
              v === "" || v === undefined || v === null ? "" : isNaN(Number(v)) ? v : Number(v);
          });
          return recomputeRow(row);
        });

        const importedMonths = new Set(imported.map((r) => r.monthId));
        setDataRows((prev) => [...prev.filter((r) => !importedMonths.has(r.monthId)), ...imported]);
      } catch (err) {
        console.error(err);
  ...
  return (
    <div className="space-y-4 font-tajawal text-slate-800 p-2" dir="rtl">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1 bg-teal-600 hover:bg-teal-700 text-white text-xs px-3 py-2 rounded shadow-sm"
            >
              <Upload className="w-4 h-4" /> استيراد Excel
            </button>
            <button
              onClick={handleExportExcel}
              className="flex items-center gap-1 bg-emerald-600 hover:bg-emerald-700 text-white text-xs px-3 py-2 rounded shadow-sm"
            >
              <Download className="w-4 h-4" /> تصدير Excel
            </button>
            <button
              onClick={handlePdf}
              className="flex items-center gap-1 bg-rose-600 hover:bg-rose-700 text-white text-xs px-3 py-2 rounded shadow-sm"
            >
              <FileText className="w-4 h-4" /> تحويل PDF
            </button>
            <button
              onClick={handlePrint}
              className="flex items-center gap-1 bg-slate-700 hover:bg-slate-800 text-white text-xs px-3 py-2 rounded shadow-sm"
            >
              <Printer className="w-4 h-4" /> طباعة
            </button>
            <button
              onClick={handleClearAll}
              className="flex items-center gap-1 bg-red-600 hover:bg-red-700 text-white text-xs px-3 py-2 rounded shadow-sm"
            >
      ...
      <div
        className="w-full overflow-x-auto border border-slate-300 shadow-sm bg-white rounded-b-lg"
                    ...
                    <tr key={row.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      {dataColumnsOrder.map((col) => {
                        ...
                        return (
                          <td
                            key={col}
                            className={`border border-slate-300 p-0.5 ${isFormula ? "bg-slate-100" : "bg-white"}`}
                          >
                            {isFormula ? (
